import {GlobalContext, RootSchema} from "../types";
import {
  comment,
  getDescendantProp,
  nodeType,
...
export function transformSchemaObj(node: any, options: TransformSchemaObjOptions): string {
  if (nodeType(node) === 'ref') {
    const ref = node.$ref;
    return _.last(ref.split('.')) ?? '';
    // node = getDescendantProp(options.rootSchema, ref);
    // if (node.enum) {
    //   node.enumName = ;
    // } else if (node.schema) {
    //   node = node.schema;
