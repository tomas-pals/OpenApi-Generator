...
const resType = (res: string | number) =>
    res === 204 || (res >= 300 && res < 400) ? 'never' : 'unknown';

export function transformResponsesObj(
    responsesObj: Record<string, any>,
    ctx: GlobalContext
): string {
    ...
    for (const httpStatusCode of Object.keys(responsesObj)) {
        ...
        if (
            (!response.content && !response.schema) ||
                ...
                for (const contentType of Object.keys(response.content)) {
                    ...
                    const responseType =
                        contentResponse && contentResponse?.schema
                            ? transformSchemaObj(contentResponse.schema, {
                                  ...ctx,
                                  required: new Set<string>()
                              })
                            : 'unknown';
